...
#include <sys/time.h>
#include <iomanip>
//using namespace std; //下面就可以不用std::了
...
int main()
{
  ...
  result=ctc_beam_search_decoder(probs_seq,beam_size,vocab,blank_id,cutoff_prob,p_score);
	
	for (int k=0;k<beam_size;k++)
	{
		std::cout<<result[k].first<<""<<result[k].second<<"\n";
	}
	std::cout<<"\nDecoding result:"<<result[0].second<<"\n";
	std::cout<<"Score:"<<result[0].first<<"\n";
	gettimeofday( &end, NULL );
	float timeuse = 1000000 * ( end.tv_sec - start.tv_sec ) + end.tv_usec - start.tv_usec;

	float t=timeuse/1000000;
	std::cout<<"Decoding time costs:"<<std::setprecision(6)<<t<<'s'<<"\n";
